...
def main():
    for i in np.arange(num_files):
        ...
        instance_name = root_folder / Path(phs_file_name.name[:-4] + '_instance')
        semantic_name = root_folder / Path(phs_file_name.name[:-4] + '_semantic')
        tifffile.imwrite(instance_name, result['instance_movie'].astype(np.int16))
        tifffile.imwrite(semantic_name, result['semantic_movie'].astype(np.int8))
