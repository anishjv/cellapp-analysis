...
class analysis:
    ...
    def __init__(self, root_folder: Path):
        ...
        ## Default parameters - needs to be moved out of here
        self.cell_params = {'HeLa':{}, 'U2OS':{}, 'RPE1':{}}
        self.erode_footprint = disk(10)
        self.min_mitotic_duration = 2 # Number of frames
        self.delta_t = 10 # default time step
    ...
    def files(self, cellaap_dir: Path):
        '''
        cellaap_dir must contain "instance" and "semnatic" tif files
        '''
        ...
        for i in np.arange(instance_shape[0]):
            pre_zoom = erosion(self.stacks["instance"][i,:,:],self.erode_footprint)
    ...
    def track_centroids(self, celltype: None, saveflag: False) -> pd.DataFrame:
        '''
        Function uses the instance segmentation file generated by cell_aap and trackpy
        to track cell centroids.
        Inputs:
        celltype  : Optional - for custom tracking parameters for each cell type
        ...
        track_table = track_table[(track_table.area < 4000) & (track_table.area > 500)]

        # 5 - max. centroid movement between frames, memory - if the segmentation flickers
        self.tracked = tp.link(track_table, 20, memory=2)
        self.tracked = tp.filter_stubs(self.tracked, 10) # min track length
    ...
    def summarize_data(self, save_flag: True):
        ...
        for id in idlist:
            ...
            _, props = find_peaks(semantic, width=self.min_mitotic_duration)
